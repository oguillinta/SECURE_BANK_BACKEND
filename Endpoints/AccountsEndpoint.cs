    ...
    public static class AccountsEndpoint
    {
        ...
        public static RouteGroupBuilder MapAccounts(this RouteGroupBuilder group)
        {
            ...
            group.MapGet("/GetByNumber/{number}", GetByNumber)
                .CacheOutput(c => c.Expire(TimeSpan.FromSeconds(60)).Tag("accounts-get"));
            group.MapGet("/GetByCustomerId/{customerId}", GetByCustomerId)
                .CacheOutput(c => c.Expire(TimeSpan.FromSeconds(60)).Tag("accounts-get"));
        ...
        static async Task<Results<Ok<List<AccountDTO>>, NotFound>> GetByCustomerId(string customerId,
            ...
            IMapper mapper)
        {
            ...
            //    return TypedResults.NotFound();
            //}
            var account = await accountsRepository.GetByCustomerId(customerId);
            if (account is null)
            {
                return TypedResults.NotFound();
            }
            var accountDTO = mapper.Map<List<AccountDTO>>(account);
            return TypedResults.Ok(accountDTO);
        }
